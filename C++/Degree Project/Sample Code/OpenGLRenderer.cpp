...
void OpenGLRenderer::Update(float dt)
{
	...
	if( ( m_selected_object = Mediator::RequestSelectedObject()) != NULL && (m_active_utility == GuiState::Utility::TRANSLATION || m_active_utility == GuiState::Utility::ROTATION || m_active_utility == GuiState::Utility::SCALE) )
	{
		...
		if (guiManager::GetInstance().getAxisOrientation(m_axis_orientation) || (m_selected_object->isRotationChanged() && m_axis_orientation == AXIS_ORIENTATION::LOCAL))
		{
...
//#define BBOX_TEST
	...
	glm::vec3 obj_cam_offset = VECTOR_TYPE_TRANSFORM::TO_GLM_3f(obj_pos - cam_center);
	...
	// 1st render pass //
	glm::mat4 model = glm::translate(glm::mat4(1.0f), VECTOR_TYPE_TRANSFORM::TO_GLM_3f(obj->getTranslationAttributes())
									 //- (obj_cam_offset * (obj_dist_center * 2.f / (179.0f)))
									 - (obj_cam_offset * l)
	) //MATRIX_TYPE_TRANSFORM::TO_GLM_MATRIX(obj->getTranslationMatrix())
	...
	glm::mat4 model_2 = glm::translate(glm::mat4(1.0f), VECTOR_TYPE_TRANSFORM::TO_GLM_3f(obj->getTranslationAttributes())
									   //- (obj_cam_offset * (obj_dist_center  * 2.f / (179.0f)))
									   - (obj_cam_offset * l)
									   - Dc )
