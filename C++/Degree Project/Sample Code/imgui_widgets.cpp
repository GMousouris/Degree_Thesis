...
bool ImGui::My_ArrowButtonEx(const char* str_id, ImGuiDir dir, ImVec2 size, ImGuiButtonFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	//size *= 0.5f;
	//const ImRect bb_draw(window->DC.CursorPos, window->DC.CursorPos + size );
	RenderMy_Arrow(bb.Min + ImVec2(ImMax(0.0f, (size.x - g.FontSize) * 0.5f), ImMax(0.0f, (size.y - g.FontSize) * 0.5f)), dir , ImVec4(0.2f,0.2f,0.2f,1.0f) );
	return pressed;
}
...
bool ImGui::Custom_Checkbox(const char* label, bool* v, bool enabled )
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
void ImGui::Dummy(float x, float y)
{
...
void ImGui::Custom_Separator(ImVec2 offset, float thickness, ImVec4 color  )
{
	ImVec2 pos = ImGui::GetCursorScreenPos() + offset;
	ImU32  col = ImGui::GetColorU32(color);
	ImGui::GetWindowDrawList()->AddLine(pos, pos + ImVec2(0, 20.0f), col, thickness);
}
void ImGui::Custom_Separator2(ImVec2 offset, float height, float thickness, ImVec4 color)
{
	ImVec2 pos = ImGui::GetCursorScreenPos() + offset;
	ImU32  col = ImGui::GetColorU32(color);
	ImGui::GetWindowDrawList()->AddLine(pos, pos + ImVec2(0, height), col, thickness);
}
...
bool ImGui::TreeNodeEx_Original(const char* label, ImGuiTreeNodeFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::TreeNodeEx(const char* label, bool is_visible, ImGuiTreeNodeFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;

	return MyTreeNodeBehavior(window->GetID(label), flags, label, NULL, is_visible);
}
bool ImGui::TreeNodeEx_2(const char* label, float width, ImGuiTreeNodeFlags flags)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;

	return MyTreeNodeBehavior_2(window->GetID(label), flags, label, NULL, width);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
	...
	float custom_width = 200.0f; // GetWorkRectMax()
	ImRect frame_bb = ImRect( window->DC.CursorPos , ImVec2( window->DC.CursorPos.x + custom_width , window->DC.CursorPos.y + frame_height) );
	if (display_frame)
	{
	...
	else
	{
		...
		if (hovered || selected)
		{
			RenderFrame(frame_bb.Min + ImVec2(25.0f, 0.0f), frame_bb.Max, col, false);
			RenderNavHighlight(frame_bb, id, nav_highlight_flags);
		}
...
bool ImGui::MyTreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, bool is_visible)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	float custom_width  = 230.0f; // GetWorkRectMax()
	ImVec2 frame_offset = ImVec2( 48.0f, 0.0f );
	ImVec2 frame_bbmin  = window->DC.CursorPos;
	ImVec2 frame_bbmax  = ImVec2( window->DC.CursorPos.x + custom_width, 
								  window->DC.CursorPos.y + frame_height );

	ImRect frame_bb = ImRect( frame_bbmin , frame_bbmax );
	if (display_frame)
	{
		// Framed header expand a little outside the default padding
		frame_bb.Min.x -= (float)(int)(window->WindowPadding.x*0.5f) - 1;
	...
	bool hov       = false;
	bool hov_arrow = false;
	hov            = IsMouseHoveringRect(frame_bb.Min + frame_offset, frame_bb.Max);
	hov_arrow      = IsMouseHoveringRect(frame_bb.Min - ImVec2(-12.0f, -3.0f), frame_bb.Min + ImVec2(12.0f, 15.0f)); //IsMouseHoveringRect(frame_bb.Min - ImVec2(-5.f,10.0f) ,  frame_bb.Min + ImVec2(15.0f,20.0f));
	//printf("\n\n hov : %i ", (int)hov);
	//printf("\n hov_arrow : %i ", (int)hov_arrow);
	if (!hov)
	{
		hovered = false;
		pressed = false;
	...
	// Render
	const ImU32 col       = GetColorU32((held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_Header : ImGuiCol_Header); // ImGuiCol_HeaderHovered
	const ImVec2 text_pos = frame_bb.Min + ImVec2(text_offset_x, text_base_offset_y);
	ImGuiNavHighlightFlags nav_highlight_flags = ImGuiNavHighlightFlags_TypeThin;
	...
	if (display_frame)
	{
		...
		RenderFrame(frame_bb.Min, frame_bb.Max, col, true, style.FrameRounding);
		RenderNavHighlight(frame_bb, id, nav_highlight_flags);
		RenderMyArrow(frame_bb.Min + ImVec2(padding.x, text_base_offset_y), is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.8f);
		if (g.LogEnabled)
		{
			// NB: '##' is normally used to hide text (as a library-wide feature), so we need to specify the text range to make sure the ## aren't stripped out here.
			const char log_prefix[] = "\n##";
	...
	else
	{
		...
		if (hovered || selected)
		{
			RenderFrame(frame_bb.Min + frame_offset , frame_bb.Max, col, false);
			RenderNavHighlight(frame_bb, id, nav_highlight_flags);
		}

		if (flags & ImGuiTreeNodeFlags_Bullet)
			RenderMyBullet(frame_bb.Min + ImVec2(text_offset_x * 0.5f, g.FontSize*0.50f + text_base_offset_y));
		else if (!is_leaf)
			RenderMyArrow(frame_bb.Min + ImVec2(padding.x, g.FontSize*0.15f + text_base_offset_y), is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.8f);
		if (g.LogEnabled)
			LogRenderedText(&text_pos, ">");
		...
		// u8"\uF208" // eye - mdi
		// u8"\uF6CF" // eye - mid - outline 
		const char * eye_end = FindRenderedTextEnd( eye );
		ImVec4 def_color = GetStyle().Colors[ImGuiCol_Text];
		
		float vv = is_visible ? 0.8f : 0.15f;
		GetStyle().Colors[ImGuiCol_Text] = def_color * vv;
		RenderMyText(text_pos, eye, eye_end, 0.4f ,  false);
		
		ImVec2 new_pos = text_pos + ImVec2( 20.0f, 0.0f );
		GetStyle().Colors[ImGuiCol_Text] = def_color;
		RenderText(new_pos, label, label_end, false);
...
bool ImGui::MyTreeNodeBehavior_2(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, float width)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	float custom_width = width;
	ImVec2 frame_offset = ImVec2(0.0f, 0.0f);
	ImVec2 frame_bbmin = window->DC.CursorPos;
	ImVec2 frame_bbmax = ImVec2(window->DC.CursorPos.x + custom_width,
								window->DC.CursorPos.y + frame_height);
	...
	{
		// Framed type
		ImVec2 offset = ImVec2(17.0f, 0.0f);
		//RenderFrame(frame_bb.Min + offset, frame_bb.Max, col, true, style.FrameRounding);
		//RenderNavHighlight(frame_bb, id, nav_highlight_flags);
		RenderMyArrow(frame_bb.Min + ImVec2(padding.x, text_base_offset_y), is_open ? ImGuiDir_Down : ImGuiDir_Right, 0.8f);
		if (g.LogEnabled)
		{
...
bool ImGui::My_Selectable(const char* label, const char * prefix , ImVec4 col,  bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...

	{
		...
		ImRect bb2 = bb;
		bb2.Max = bb.Min + ImVec2(20.0f, bb.Max.y);
		
		GetStyle().Colors[ImGuiCol_Text] = col;
		RenderTextClipped(bb_inner.Min, bb_inner.Min + ImVec2(5.0f,5.0f) , prefix, NULL, &CalcTextSize(prefix, NULL, true), style.SelectableTextAlign, &bb2);
		GetStyle().Colors[ImGuiCol_Text] = def_color;
	}
	if (flags & ImGuiSelectableFlags_Disabled) PopStyleColor();
...
bool ImGui::My_Selectable2(const char* label, const char * prefix, ImVec4 col_t, ImVec4 col, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	ImVec4 def_color = GetStyle().Colors[ImGuiCol_Text];
	ImGui::GetStyle().Colors[ImGuiCol_Text] = col_t;
	RenderTextClipped(bb_inner.Min, bb_inner.Max, label, NULL, &label_size, style.SelectableTextAlign, &bb);
	ImGui::GetStyle().Colors[ImGuiCol_Text] = def_color;
	{
...
bool    ImGui::BeginTabItem2(const char* label, const char* t_label, bool* p_open, ImGuiTabItemFlags flags)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	if (window->SkipItems)
	...
	bool ret = TabItemEx2(tab_bar, label, t_label, p_open, flags);
	if (ret && !(flags & ImGuiTabItemFlags_NoPushId))
	{
		ImGuiTabItem* tab = &tab_bar->Tabs[tab_bar->LastTabItemIdx];
		PushOverrideID(tab->ID); // We already hashed 'label' so push into the ID stack directly instead of doing another hash through PushID(label)
...
bool    ImGui::TabItemEx2(ImGuiTabBar* tab_bar, const char* label, const char* t_label, bool* p_open, ImGuiTabItemFlags flags)
{
	// Layout whole tab bar if not already done
	if (tab_bar->WantLayout)
		TabBarLayout(tab_bar);
	...
	if (g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f && IsItemHovered())
		if (!(tab_bar->Flags & ImGuiTabBarFlags_NoTooltip))
			SetTooltip("%.*s", (int)(FindRenderedTextEnd(t_label) - t_label), t_label);
