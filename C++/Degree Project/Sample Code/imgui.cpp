...
void ImGui::RenderMyText(ImVec2 pos, const char* text, const char* text_end, float scale, bool hide_text_after_hash)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	...
	if (text != text_display_end)
	{
		window->DrawList->AddText(g.Font, g.FontSize + scale , pos, GetColorU32(ImGuiCol_Text), text, text_display_end);
		if (g.LogEnabled)
			LogRenderedText(&pos, text, text_display_end);
	}
}
...
void ImGui::RenderMy_Arrow(ImVec2 p_min, ImGuiDir dir , ImVec4 color , float scale )
{
	ImGuiContext& g = *GImGui;
	...
	g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(color) );

	center -= ImVec2(3.0f, 0.0f);
	g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(ImGuiCol_PopupBg));
}

void ImGui::RenderMyArrow(ImVec2 p_min, ImGuiDir dir, float scale)
{
	ImGuiContext& g = *GImGui;
	...
	case ImGuiDir_Down:
		if (dir == ImGuiDir_Up) r = -r;
		a = ImVec2(+0.000f, +0.750f)  * 0.25f  * r;
		b = ImVec2(-0.866f, -0.750f)  * 1.50f  * r;
		c = ImVec2(+0.866f, -0.750f)  * 1.50f  * r;
		break;
	case ImGuiDir_Left:
	case ImGuiDir_Right:
		if (dir == ImGuiDir_Left) r = -r;
		a = ImVec2(+0.750f, +0.000f) * 0.35f  * r;
		b = ImVec2(-0.750f, +0.866f) * 1.25f  * r;
		c = ImVec2(-0.750f, -0.866f) * 1.25f  * r;
		break;
	case ImGuiDir_None:
	case ImGuiDir_COUNT:
		IM_ASSERT(0);
	...
	g.CurrentWindow->DrawList->AddTriangleFilled(center + a, center + b, center + c, GetColorU32(ImGuiCol_Text));
	g.CurrentWindow->DrawList->AddTriangle(center + a*1.1, center + b*1.1, center + c*1.1, GetColorU32(ImVec4(0.15f, 0.15f, 0.15f, 1.0f)));
}
...
void ImGui::RenderMyBullet(ImVec2 pos)
{
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = g.CurrentWindow;
	window->DrawList->AddCircleFilled(pos, g.FontSize*0.3f, GetColorU32(ImGuiCol_TextDisabled), 20);
	window->DrawList->AddCircle(      pos, g.FontSize*0.32f, GetColorU32(ImVec4(0.1f,0.1f,0.1f,1.0f)), 30);
}
...
static void ImGui::NavUpdate()
{
    ...
    if (nav_keyboard_active)
    {
		...
		NAV_MAP_KEY(ImGuiKey_KP_ENTER,  ImGuiNavInput_Input    );
